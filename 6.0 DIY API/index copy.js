...
app.get("/jokes/:id",(req,res)=>{
 ...
 const getjoke = jokes.find((joke)=> joke.id === getID)
 res.json(getjoke)
})
...
app.get("/filter",(req,res)=>{
  ...
  const getType = jokes.find((joke)=> joke.jokeType === Type)
  res.json(getType)
});
//4. POST a new joke
...
app.patch("/jokes/:id",(req,res)=>{
  ...
  // we can also use if-else, but this looks clean
});
...
app.delete("/all",(req,res)=>{
  ...
  if(userKey === masterKey){
    ...
    res
    .status(404)
    .json({error: "not an authorised member nigga"});
  }
})
app.listen(port, () => {
